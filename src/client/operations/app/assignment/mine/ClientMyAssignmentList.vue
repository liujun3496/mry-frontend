  ...
  <div :class="$style.wrapper">
    <template v-if="isAssignmentAllowed">
      <template v-if=" initialised">
        ...
        <ClientMyAssignmentListFilterDrawer :shouldShowGroupSelect="shouldShowGroupSelect"
                                            :show.sync="filterDrawerVisible">
        </ClientMyAssignmentListFilterDrawer>
      </template>
    </template>
...
import ClientMyAssignmentListFilterDrawer
  from '@/client/operations/app/assignment/mine/ClientMyAssignmentListFilterDrawer';
...
export default {
  ...
  computed: {
    ...
    ...clientMyAssignmentListStore.mapGetters(['hasFilter']),
  ...
  methods: {
    ...clientMyAssignmentListStore.mapActions([
    ...
    goToAssignmentQrs(assignment) {
      if (this.originGroupId === 'ALL') {
        this.$router.push({name: 'client-my-assignment-qr-list', params: {assignmentId: assignment.id}});
      } else {
        this.$router.push({
